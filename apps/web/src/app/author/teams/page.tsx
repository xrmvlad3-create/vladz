...
async function createTeam(formData: FormData): Promise<void> {
  ...
  const team = await prisma.team.upsert({
    where: { slug },
    update: { name, ownerId: me.id },
    create: { name, slug, ownerId: me.id }
  });

  await prisma.teamMember.upsert({
    where: { teamId_userId: { teamId: team.id, userId: me.id } },
    update: { role: "owner" },
    create: { teamId: team.id, userId: me.id, role: "owner" }
  });
...
export default async function AuthorTeamsPage() {
  ...
  if (!session) {
    return (
      <main>
        <h2 style={{ fontSize: 18, fontWeight: 600 }}>Echipele mele</h2>
        <p style={{ marginTop: 8 }}>
          Te rugăm să te <a href="/login?callbackUrl=/author/teams" style={{ color: "#2563eb" }}>autentifici</a>.
        </p>
      </main>
    );
  }
  ...
  const me = await prisma.user.findUnique({ where: { email: session.user?.email || "" } });
  if (!me) {
    return <main><p>Eroare utilizator.</p></main>;
  }
  ...
  return (
    <main>
      <h2 style={{ fontSize: 18, fontWeight: 600, marginBottom: 12 }}>Echipele mele</h2>

      <section style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 12, background: "#fff", marginBottom: 12 }}>
        <h3 style={{ fontWeight: 600, marginBottom: 8 }}>Creează/actualizează echipă</h3>
        <form action={createTeam} style={{ display: "grid", gap: 8, gridTemplateColumns: "repeat(3, minmax(0,1fr))" }}>
          <input name="name" placeholder="Nume echipă" style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 10px" }} />
          <input name="slug" placeholder="Slug (ex: grup-derma)" style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 10px" }} />
      ...
      {myTeams.map(t => (
        <section key={t.slug} style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 12, background: "#fff", marginBottom: 12 }}>
          ...
          <ul style={{ listStyle: "none", paddingLeft: 0, marginTop: 8 }}>
            {t.members.map(m => (
              <li key={m.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: "1px solid #f3f4f6" }}>
                ...
                {t.owner.email === (session.user?.email || "") && (
