...
async function addSpecialty(formData: FormData): Promise<void> {
  "use server";
  if (!warnIfNoDatabase("admin:catalog:addSpecialty")) return;
...
async function addCondition(formData: FormData): Promise<void> {
  "use server";
  if (!warnIfNoDatabase("admin:catalog:addCondition")) return;
...
async function addProcedure(formData: FormData): Promise<void> {
  "use server";
  if (!warnIfNoDatabase("admin:catalog:addProcedure")) return;
...
async function updateSpecialty(formData: FormData): Promise<void> {
  "use server";
  if (!warnIfNoDatabase("admin:catalog:updateSpecialty")) return;
...
async function updateCondition(formData: FormData): Promise<void> {
  "use server";
  if (!warnIfNoDatabase("admin:catalog:updateCondition")) return;
...
async function updateProcedure(formData: FormData): Promise<void> {
  "use server";
  if (!warnIfNoDatabase("admin:catalog:updateProcedure")) return;
...
async function deleteSpecialty(formData: FormData): Promise<void> {
  "use server";
  if (!warnIfNoDatabase("admin:catalog:deleteSpecialty")) return;
...
async function deleteCondition(formData: FormData): Promise<void> {
  "use server";
  if (!warnIfNoDatabase("admin:catalog:deleteCondition")) return;
...
async function deleteProcedure(formData: FormData): Promise<void> {
  "use server";
  if (!warnIfNoDatabase("admin:catalog:deleteProcedure")) return;
...
function pagingParams(total: number, pageParam: string | undefined, q: string | null, defaultSize = 20): Paging {
...
function pager(sectionKey: "s" | "c" | "p", page: number, pages: number, qParam: string, qVal: string) {
  ...
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 8 }}>
      <a href={`${PATH}?${qsPrev}`} style={{ color: "#2563eb", opacity: page <= 1 ? 0.5 : 1 }}>&larr;</a>
      <span style={{ color: "#6b7280", fontSize: 12 }}>pagina {page} / {pages}</span>
      <a href={`${PATH}?${qsNext}`} style={{ color: "#2563eb", opacity: page >= pages ? 0.5 : 1 }}>&rarr;</a>
    </div>
  );
}
  ...
  if (!session || role !== "admin") {
    return (
      <main>
        ...
        <p style={{ marginTop: 8 }}>
          Te rugam sa te <a href="/login?callbackUrl=/admin/catalog" style={{ color: "#2563eb" }}>autentifici</a> ca administrator.
        </p>
      </main>
    );
  }
  ...
  if (!hasDatabaseUrl()) {
    return (
      <main>
        ...
        <p style={{ color: "#b91c1c", marginBottom: 12 }}>{missingDatabaseMessage()}</p>
        <p style={{ color: "#6b7280" }}>
          Conecteaza un Postgres (Neon, Supabase sau Vercel Postgres) si ruleaza <code>npx prisma migrate deploy</code> urmat de <code>npm run seed</code> pentru a popula baza de date.
        </p>
      </main>
    );
  }
    ...
    <main>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0,1fr))", gap: 12 }}>
        <div style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 12, background: "#fff" }}>
          <h3 style={{ fontWeight: 600, marginBottom: 8 }}>Specialitati</h3>
          <form action={addSpecialty} style={{ display: "grid", gap: 8, marginBottom: 12 }}>
            ...
            <button style={{ background: "#111827", color: "#fff", borderRadius: 8, padding: "8px 10px", fontWeight: 600 }}>Creeaza/Actualizeaza</button>
          </form>

          <form method="GET" style={{ display: "flex", gap: 8, marginBottom: 8 }}>
            <input name="spq" defaultValue={spQ} placeholder="Cautare specialitati" style={{ flex: 1, border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 10px" }} />
            <button style={{ background: "#111827", color: "#fff", borderRadius: 8, padding: "8px 10px", fontWeight: 600 }}>Cauta</button>
          </form>
            ...
            {specialties.map((s) => (
              <li key={s.id} style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: 8, marginBottom: 6 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                  <div>
                    <strong>{s.name}</strong> <span style={{ color: "#6b7280" }}>({s.slug})</span>
                  </div>
                ...
                <form action={updateSpecialty} style={{ display: "flex", gap: 6, marginTop: 6 }}>
                  ...
                  <button style={{ background: "#111827", color: "#fff", borderRadius: 8, padding: "6px 10px", fontWeight: 600 }}>Salveaza</button>
                </form>
              </li>
            ))}
          </ul>
        ...
        <div style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 12, background: "#fff" }}>
          <h3 style={{ fontWeight: 600, marginBottom: 8 }}>Afectiuni</h3>
          <form action={addCondition} style={{ display: "grid", gap: 8, marginBottom: 12 }}>
            ...
            <input name="name" placeholder="nume (ex: Acnee vulgara)" style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 10px" }} />
          ...
          <form method="GET" style={{ display: "flex", gap: 8, marginBottom: 8 }}>
            <input name="cpq" defaultValue={cpQ} placeholder="Cautare afectiuni" style={{ flex: 1, border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 10px" }} />
            <button style={{ background: "#111827", color: "#fff", borderRadius: 8, padding: "8px 10px", fontWeight: 600 }}>Cauta</button>
          </form>
          ...
          <ul style={{ listStyle: "none", paddingLeft: 0 }}>
            {conditions.map((c) => (
              <li key={c.id} style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: 8, marginBottom: 6 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                  <div>
                    <strong>{c.name}</strong> <span style={{ color: "#6b7280" }}>({c.slug})</span> - {c.specialty?.name ?? "General"}
                    {c.isCommon ? <span style={{ marginLeft: 6, color: "#92400E", fontSize: 12 }}>Comun</span> : null}
                  </div>
        ...
        <div style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 12, background: "#fff" }}>
          ...
          <form method="GET" style={{ display: "flex", gap: 8, marginBottom: 8 }}>
            <input name="ppq" defaultValue={ppQ} placeholder="Cautare proceduri" style={{ flex: 1, border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 10px" }} />
            <button style={{ background: "#111827", color: "#fff", borderRadius: 8, padding: "8px 10px", fontWeight: 600 }}>Cauta</button>
          </form>
          ...
          <ul style={{ listStyle: "none", paddingLeft: 0 }}>
            {procedures.map((p) => (
              <li key={p.id} style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: 8, marginBottom: 6 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                  <div>
                    <strong>{p.name}</strong> <span style={{ color: "#6b7280" }}>({p.slug})</span> - {p.specialty?.name ?? "General"}
                  </div>
